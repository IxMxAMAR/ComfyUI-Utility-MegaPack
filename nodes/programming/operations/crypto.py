...
_HKDF_SALT = b"utility-megapack/aes-gcm/v1"


def _derive_aes_key(key_str: str) -> bytes:
    """Derive a 32-byte AES key from an arbitrary-length string via HKDF."""
    return HKDF(
        algorithm=hashes.SHA256(),
        length=32,
        salt=_HKDF_SALT,
        info=b"aes-gcm key",
    ).derive(key_str.encode("utf-8"))
...
@op(
    ...
    output_indices=(0,),
    description="AES-256-GCM. Returns base64(nonce[12] || ciphertext_with_tag).",
)
def aes_encrypt(self, plaintext, key):
    aes = AESGCM(_derive_aes_key(key))
    nonce = os.urandom(12)
...
@op(
    ...
    output_indices=(0,),
    description="Inverse of aes_encrypt. Raises on tag mismatch.",
)
